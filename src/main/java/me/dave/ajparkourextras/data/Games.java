...
public class Games {
    ...
    public int getHighScore(UUID uuid, String area) {
        ...
        try {
            ...
            ResultSet resultSet = conn.createStatement().executeQuery(
                "select score from " + extrasTablePrefix + "games where player='" + uuid.toString() + "' and area='" + area + "' and epochTime>=" + epochLowerBound + " order by score desc limit 1"
            );
    ...
    public TopEntry getTopPosition(int position, String area) {
        ...
        try {
            ...
            long epochLowerBound = Instant.now().minus(AJParkourExtras.getInstance().getConfigManager().getLeaderboardRange(), ChronoUnit.DAYS).getEpochSecond();
            ResultSet resultSet = conn.createStatement().executeQuery(
                "select a.* from " + extrasTablePrefix + " a inner join (select c.player, c.score, max(c.epochtime) as max_epochtime from " + extrasTablePrefix + "games c inner join (select player, max(score) as max_score from " + extrasTablePrefix + "games where area = '" + area + "' and epochtime >= " + epochLowerBound + " group by player) d on c.player = d.player and c.score = d.max_score where c.area = '" + area + "' and c.epochtime >= " + epochLowerBound + " group by c.player, c.score) b on a.player = b.player and a.score = b.score and a.epochtime = b.max_epochtime where a.area = '" + area + "' and a.epochtime >= " + epochLowerBound + " order by a.score desc limit " + (position-1) + "," + position
